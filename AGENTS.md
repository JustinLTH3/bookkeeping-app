...
- Use `npm run test:integration` for DB-backed integration tests — requires the test database: `docker compose up -d` (postgres:16 on port 5433, or set `TEST_DATABASE_URL`). Migrations are applied automatically by the Vitest global setup.
